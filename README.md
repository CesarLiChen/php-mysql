...
[Workflow after setup is complete](#normal-workflow)  
...
### Normal Workflow
- Start Abyss server:
  - In terminal, `cd Documents/abyssws/` then start it by `./abyssws`
- Start MySQL server:
  - In terminal, `systemctl start mysqld`
  - `systemctl status mysqld` to see that it started successfully.
